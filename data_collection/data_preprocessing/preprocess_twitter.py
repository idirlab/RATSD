"""
This file processes the Twitter dataset available for RumorEval 2019 into data frames and stores them into CSV files.
...
import json
import numpy as np
from data_collection.data_preprocessing.constants import (
    path_train_key,
    path_dev_key,
    path_test_key,
    training_data_path,
...
def processTwitterKeyDataFrame(key_df, datasetType):
    ...
    key_taska_df = key_taska_df.rename(
        columns={"index": "id", "subtaskaenglish": "label"}
    )

    if datasetType == "train":
        twitter_key_tasks_df = key_taska_df[
            0:4519
        ]  # Records after 4519 are keys for Reddit posts
    elif datasetType == "dev":
        twitter_key_tasks_df = key_taska_df[
            0:1049
        ]  # Records after 1049 are keys for Reddit posts
    elif datasetType == "test":
        twitter_key_tasks_df = key_taska_df[
            0:1066
        ]  # Records after 1066 are keys for Reddit posts
...
def processTwitterSourcePosts(twitter_dataset_path):
    twitter_dirs = glob(twitter_dataset_path + "/*")
    ...
    for directory in twitter_dirs_sorted:
        twitter_src_dirs.append(glob(directory + "/*"))
    ...
    for src_tweet_file in src_tweet_files_sorted:
        path = glob(src_tweet_file + "/source-tweet/*")[0]
        tweet_post_dict = {}
...
to a Dataframe"""
twitter_trainDev_dirs_sorted, twitter_trainDev_src_posts = processTwitterSourcePosts(
    twitter_trainingDev_data_path
)
twitter_trainDev_src_posts_df = pd.DataFrame(twitter_trainDev_src_posts)

twitter_test_dirs_sorted, twitter_test_src_posts = processTwitterSourcePosts(
    twitter_test_data_path
)
...
def processTwitterReplyPosts(twitter_dataset_path, twitter_dirs_sorted):
    twitter_replies = []
    # print("z", twitter_dirs_sorted)
    for directory in twitter_dirs_sorted:
        src_path = glob(directory + "/*")
        for path in src_path:
            for p in glob(path + "/replies/*"):
                tweet_post_dict = {}
                with open(p) as f:
                    for line in f:
                        src = json.loads(line)
                        ...
                        tweet_post_dict[
                            "inre"
                        ] = inre  # Loading ID of the post that this post replied to
                        tweet_post_dict[
                            "source"
                        ] = directory  # Laoding the ID of the source post of this conversation thread
...
twitter_trainDev_replies = processTwitterReplyPosts(
    twitter_trainingDev_data_path, twitter_trainDev_dirs_sorted
)
twitter_trainDev_replies_df = pd.DataFrame(twitter_trainDev_replies)

twitter_test_replies = processTwitterReplyPosts(
    twitter_test_data_path, twitter_test_dirs_sorted
)
...
def twitterCleanDf(src_posts_df, replies_df):
    ...
    # df10
    twitter_clean_data.id = (
        twitter_clean_data.id.str.strip()
    )  # Strips out any white spaces in the id column
    twitter_clean_data.inre = (
        twitter_clean_data.inre.str.strip()
    )  # Strips out any white spaces in the inre column
    return twitter_clean_data


# Loading the clean twitter dataframes for training, development and test data
twitter_clean_trainDev_df = twitterCleanDf(
    twitter_trainDev_src_posts_df, twitter_trainDev_replies_df
)
twitter_clean_test_df = twitterCleanDf(
    twitter_test_src_posts_df, twitter_test_replies_df
)
...
twitter_train_withKeys_df = pd.merge(
    twitter_clean_trainDev_df, twitter_train_key_df, how="inner", on="id",
)
twitter_dev_withKeys_df = pd.merge(
    twitter_clean_trainDev_df, twitter_dev_key_df, how="inner", on="id",
)
twitter_test_withKeys_df = pd.merge(
    twitter_clean_test_df, twitter_test_key_df, how="inner", on="id",
)
...
def fetchTwitterDataset(twitter_withKeys_df):
    ...
    twitter_dataset = pd.merge(
        twitter_withKeys_df, twitter_df_new, how="left", on="inre",
    )
    twitter_dataset1 = pd.merge(
        twitter_withKeys_df, twitter_df_new1, how="left", on="source",
    )
...
twitter_train_dataset_inre, twitter_train_dataset_src = fetchTwitterDataset(
    twitter_train_withKeys_df
)
twitter_dev_dataset_inre, twitter_dev_dataset_src = fetchTwitterDataset(
    twitter_dev_withKeys_df
)
twitter_test_dataset_inre, twitter_test_dataset_src = fetchTwitterDataset(
    twitter_test_withKeys_df
)

# Merging the 2 dataframes containig source text and reply text for training, dev and test data
twitter_train_dataset_src = pd.merge(
    twitter_train_dataset_inre, twitter_train_dataset_src, how="inner", on="id",
)
twitter_dev_dataset_src = pd.merge(
    twitter_dev_dataset_inre, twitter_dev_dataset_src, how="inner", on="id",
)
twitter_test_dataset_src = pd.merge(
    twitter_test_dataset_inre, twitter_test_dataset_src, how="inner", on="id",
)

# Removing unnecessary columns for twitter training, dev and test data
twitter_new_train_data_df = twitter_train_dataset_src[
    ["text_x", "id", "inre_x", "source_x", "label_x", "inreText", "sourceText"]
].copy()
twitter_new_dev_data_df = twitter_dev_dataset_src[
    ["text_x", "id", "inre_x", "source_x", "label_x", "inreText", "sourceText"]
].copy()
twitter_new_test_data_df = twitter_test_dataset_src[
    ["text_x", "id", "inre_x", "source_x", "label_x", "inreText", "sourceText"]
].copy()
...
twitter_new_train_data_df.to_csv(
    "./data_collection/dataset/TwitterTrainDataSrc.csv", encoding="utf-8", index=False
)
twitter_new_dev_data_df.to_csv(
    "./data_collection/dataset/TwitterDevDataSrc.csv", encoding="utf-8", index=False
)
twitter_new_test_data_df.to_csv(
    "./data_collection/dataset/TwitterTestDataSrc.csv", encoding="utf-8", index=False
)
